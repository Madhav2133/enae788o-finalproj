'''
This script is an extension of Modified CSA - SAR script by implementing a Finite State Machine on top of it.
'''
...
# ---------- SAR with FSM ----------
def run_fsm_sar(target_cells=None):
    # Initialization
    ...
    ax.set_title("FSM SAR Simulation")
    ax.set_aspect('equal')
    ...
    for t in range(STEPS):
        ...
        for i in range(NUM_AGENTS):
            ...
            near_target = any(
                np.linalg.norm(positions[i] - (tx * CELL_SIZE + CELL_SIZE / 2, ty * CELL_SIZE + CELL_SIZE / 2)) < CELL_SIZE
                for j, (tx, ty) in enumerate(target_cells) if not found_targets[j]
            )
            if states[i] != "rescue":
                states[i] = "mission" if near_target else "search"

            if states[i] == "rescue":
                new_vel[i] = np.zeros(2)
                continue
            elif states[i] == "mission":
                acc = np.random.uniform(-0.5, 0.5, 2)  # small random drift
            else:  # search
                acc = (
                    1.5 * separation(i, positions) +
                    1.0 * alignment(i, positions, velocities) +
                    1.2 * cohesion(i, positions) +
                    1.5 * goal_force(positions[i], target_cells, found_targets)
                )
        ...
        state_colors = {"search": "blue", "mission": "yellow", "rescue": "green"}
        scatter.set_offsets(positions)
        scatter.set_color([state_colors[s] for s in states])
...
def show_final_plots(agent_tracks, coverage_over_time):
    ...
    plt2.title("Coverage Over Time (FSM SAR)")
    plt2.grid(True)
    plt2.savefig("fsm_coverage_over_time.png")
    ...
    plt2.xlabel("X")
    plt2.ylabel("Y")
    plt2.title("Agent Trajectories (FSM SAR)")
    plt2.xlim(0, WORLD_SIZE)
    plt2.ylim(0, WORLD_SIZE)
    plt2.gca().set_aspect('equal')
    plt2.grid(True)
    plt2.savefig("fsm_agent_trajectories.png")
